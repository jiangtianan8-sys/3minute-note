...
||commit;|
|select id,class_name,teacher_id from class_teacher where teacher_id=1;<br><br>读到了事务 B 修改的数据，和第一次查询的结果不一样，是不可重读的。||
|commit;||
...
|select id,class_name,teacher_id from class_teacher where teacher_id=1;<br><br>没有读到事务 B 修改的数据，和第一次 sql 读取的一样，是可重复读的。<br><br>没有读到事务 C 新添加的数据。|||
|commit;|||
...
![](assets/Innodb中的事务隔离级别和锁的关系/file-20251124152307014.png)
