...
  所以当你用 postman 测试一个分页查询接口时，发现第一次打印耗时 300 ~ 400ms，往后不停的查找下一页都是 30 ~ 40ms，原因就是第一次请求接口时，读数据库的时候需要读磁盘，从磁盘加载 16KB 的数据到内存，往后下一页的数据都是从内存中获取，没有再读磁盘，除非在内存中的 16KB 的数据中找不到，才会再次读磁盘获取下一个 16KB 的数据到内存中。（我们不讨论 mysql 8.0 舍弃的查询缓存特性，我测试过 mysql 5.7 中关闭了查询缓存，也仍然是第一次慢，后续查询很快，查询时间相差大概 10 倍的样子）

温馨提示：分页查询和数据库的一页 16KB 中的 " 页 " 是两个概念。

![](assets/MySQL的varchar水真的太深了——InnoDB记录存储结构/file-20251124112539564.png)
...
1 SHOW VARIABLES LIKE "innodb_default_row_format"

![](assets/MySQL的varchar水真的太深了——InnoDB记录存储结构/file-20251124112605969.png)
...
![](assets/MySQL的varchar水真的太深了——InnoDB记录存储结构/file-20251124112708096.png)

3.1 dynamic——innodb 默认行格式

![](assets/MySQL的varchar水真的太深了——InnoDB记录存储结构/file-20251124112721704.png)
...
1 SELECT LENGTH(c2) from test where c1='aaaa';

![](assets/MySQL的varchar水真的太深了——InnoDB记录存储结构/file-20251124112738234.png)

![](assets/MySQL的varchar水真的太深了——InnoDB记录存储结构/file-20251124112748390.png)
...
![](assets/MySQL的varchar水真的太深了——InnoDB记录存储结构/file-20251124112801243.png)
...
对于变长类型 VARCHAR(M) 来说，这种类型表示能存储最多 M 个字符（注意是字符不是字节）
...
1 create table t2 ( name varchar(16383))charset=utf8mb4;

![](assets/MySQL的varchar水真的太深了——InnoDB记录存储结构/file-20251124112818081.png)

不断往这个字段添加字符保存测试，最后发现，这些字符总长度到极限也就是 48545 字节。

![](assets/MySQL的varchar水真的太深了——InnoDB记录存储结构/file-20251124112837826.png)

如果超过就会报错

![](assets/MySQL的varchar水真的太深了——InnoDB记录存储结构/file-20251124112848348.png)
...
![](assets/MySQL的varchar水真的太深了——InnoDB记录存储结构/file-20251124112902843.png)

对于第二条记录，c4 列值为 NULL，所以只存储 c1 和 c2 列即可。

![](assets/MySQL的varchar水真的太深了——InnoDB记录存储结构/file-20251124112912412.png)
...
如果表中没有允许存储 NULL 的列，则 NULL 值列表也不存在了，否则将每个允许存储 NULL 的列对应一个二进制位，二进制位按照列的顺序逆序排列。二进制位的值为 1 时，代表该列的值为 NULL，为 0 时，代表该列的值不为 NULL。因为表 test 的 c1、c3、c4 都是允许存储 NULL 值的允许为 NULL 的列，所以这 3 个列和二进制位的对应关系就是这样：

![](assets/MySQL的varchar水真的太深了——InnoDB记录存储结构/file-20251124112927846.png)

NULL 值列表必须用整数个字节的位表示，如果使用的二进制位个数不是整数个字节，则在字节的高位补 0。

也就是说，表 test 只有 3 个字段允许为 NULL，对应 3 个二进制位，不足 1 字节，那么就在高位补 0 即可。

![](assets/MySQL的varchar水真的太深了——InnoDB记录存储结构/file-20251124112938043.png)

以此类推，如果表中有 9 个字段都允许为 NULL，那么这个记录的 NULL 值列表就需要 2 个字节来表示，高字节高位补 0。

对于第一条记录，c1、c3、c4 都不为 NULL，对应的为进制位为 0，十六进制表示就是 0x00

![](assets/MySQL的varchar水真的太深了——InnoDB记录存储结构/file-20251124112955226.png)

对于第二条记录，c3、c4 都是 NULL，对应的二进制位为 1，十六进制表示就是 0x06

![](assets/MySQL的varchar水真的太深了——InnoDB记录存储结构/file-20251124113010163.png)

这两条记录在填充了 NULL 值列表后示意图如下：

![](assets/MySQL的varchar水真的太深了——InnoDB记录存储结构/file-20251124113021337.png)
...
后续：如果大家对 innodb 存储结构其他行格式感兴趣，或者我没说的记录头信息，可以去阅读《MySQL 是怎样运行的》一书，我和书中不同的是，书中讲的 Compact 格式，字符集是 ascii，我选用的是平时开发中用到的默认 dynamic 格式，字符集是 utf8mb4，字符集变化后所有的数据我在文中和图中都有重新计算。大家平时或许没关注过行格式，那么就是按照 dynamic 格式理解就可以，更贴近实际开发。

————————————————

版权声明：本文为 CSDN 博主「砖业洋 __」的原创文章，遵循 CC 4.0 BY-SA 版权协议，转载请附上原文出处链接及本声明。

原文链接：https://blog.csdn.net/qq_34115899/article/details/117524328
