...
通过 cat /etc/passwd 可以查看用户的 pass
...
4、如果一个用户同时属于多个用户组，那么用户可以在用户组之间切换，以便具有其他用户组的权限。用户可以在登录后，使用命令 newgrp 切换到其他用户组，这个命令的参数就是目的用户组。 例如：
...
4.修改/etc/passwd 即可，把用户名的 ID 和 ID 组修改成 0。

来源： [http://www.jb51.net/article/45848.htm](http://www.jb51.net/article/45848.htm)
